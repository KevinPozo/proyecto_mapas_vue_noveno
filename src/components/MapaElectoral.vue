...
import provinciasData from "@/assets/1996/Datos/PrimeraVuelta/Datos2025Provincias.json";
import cantonesData from "@/assets/1996/Datos/PrimeraVuelta/Datos2025Cantones.json";
import parroquiasData from "@/assets/1996/Datos/PrimeraVuelta/Datos2025Parroquias.json";
...
export default {
  ...
  data() {
    return {
      datosExportacion: [...provinciasData, ...cantonesData, ...parroquiasData],
      select: "",
      select2: "",
      type: "1",
      types: ["1", "2"],
  ...
  computed: {
    provs() {
      ...
      return this.geoProvincias.features
    ...
    cantons() {
      ...
      const provFeature = this.geoProvincias.features.find(
        (f) => f.properties.PROVINCIA === this.select
      );
      ...
      return this.geoCantones.features
        .filter(
          (f) => String(f.properties.CODPRO).replace(/^0+/, "") === idProv
        )
  ...
  methods: {
    ...
    datosBusqueda2() {
      ...
      if (this.select == "") {
        ...
        if (provFeature) {
          const idProv = String(provFeature.properties.CODPRO).replace(
            /^0+/,
            ""
          );
          const cantonFeature = this.geoCantones.features.find(
            (f) =>
              String(f.properties.CODPRO).replace(/^0+/, "") === idProv &&
              (f.properties.CANTON === this.select2 ||
                f.properties.NOM_CAN === this.select2)
          );

          if (cantonFeature) {
            const idCan = String(cantonFeature.properties.CODCAN).replace(
              /^0+/,
              ""
            );

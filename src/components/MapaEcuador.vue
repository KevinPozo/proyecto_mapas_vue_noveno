  ...
  methods: {
    ...
    getDataForLevel(level, parentId = null) {
      ...
      if (level === "PAIS") {
        ...
        dataArray = this.resultadosCantones.filter(
          (d) => this.normalizeId(d.CODPRO) === this.normalizeId(parentId)
        );
        idField = "CODCAN";
        nameField = "CANTON";
      } else if (level === "CANTON") {
        dataArray = this.resultadosParroquias.filter(
          (d) => this.normalizeId(d.CODCAN) === this.normalizeId(parentId)
        );
        idField = "CODPAR";
        nameField = "PARROQUIA";
      }
      ...
      return dataArray.map((item) => {
        ...
        if (
          ...
          const baseColor = this.colores[partidoFilter]
            ? this.colores[partidoFilter].principal
            : "#666666";
          fill = baseColor;
        } else {
          ...
          const colorInfo = this.colores[winnerName] || {
            principal: "#cccccc",
          };
...
Votos: {winnerVotes} ({winnerPercent}%)`;
    ...
    handleExternalFilter() {
      ...
      if (idProv && idCan) {
        ...
        const provPolygon = this.paisSeries.getPolygonById(
          this.normalizeId(idProv)
        );
        if (provPolygon) {
          ...
          setTimeout(() => {
            const canPolygon = this.provinciaSeries.getPolygonById(
              this.normalizeId(idCan)
            );
            if (canPolygon) {
        ...
        this.resetMapToHome();
        const polygon = this.paisSeries.getPolygonById(
          this.normalizeId(idProv)
        );
        if (polygon) {
    ...
    drillDownToCanton(dataContext) {
      try {
        ...
        let cantonParroquias = features.filter((f) => {
          return (
            this.normalizeId(f.properties.CODCAN) === this.normalizeId(idCanton)
          );
        });
    ...
    flattenElectionData(data) {
      ...
      return data.map((item) => {
        let locationName =
          item.PROVINCIA || item.CANTON || item.PARROQUIA || "Desconocido";
        ...
        if (item.resultados) {
          const sortedCandidates = Object.values(item.resultados).sort(
            (a, b) => b.votos - a.votos
          );
    ...
    descargarHTML() {
      ...
      let html =
        "<html><head><style>table, th, td { border: 1px solid black; border-collapse: collapse; padding: 5px; font-family: Arial; } th { background-color: #f2f2f2; }</style></head><body><h2>Resultados Electorales</h2><table>";

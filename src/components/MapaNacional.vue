        ...
        <v-card outlined color="transparent" width="100%">
          ...
          <v-row>
            <v-col cols="12" md="9">
              <div v-if="(!expand)">
                <span :key="id_codMapa">
                  <MapaEcuador :id_1=id_codMapa :datos="mockDatos" @mapaTitulo="(i) => tituloMapa = i" />
                </span>
              </div>
              <div v-else>
                <span v-bind:key="id_codMapa">
                  <MapaEcuador :id_1=id_codMapa :datos="mockDatos" />
                </span>
              </div>
            </v-col>
...
const DUMMY_PROVINCIAS = [
  { provincia: 'AZUAY', id: '01', ncantones: ['Cuenca', 'Gualaceo'], cantones: [{ nombre: 'Cuenca', idcan: '01' }, { nombre: 'Gualaceo', idcan: '02' }] },
  { provincia: 'PICHINCHA', id: '17', ncantones: ['Quito', 'Cayambe'], cantones: [{ nombre: 'Quito', idcan: '01' }, { nombre: 'Cayambe', idcan: '02' }] }
];
...
export default {
  ...
  methods: {
    ...
    datosBusqueda2() {
      ...
      let condicionPartido = politico && politico.nombrePartido == 'Resultados Generales' ? "" : (politico ? politico.json : "");
    ...
    asignarTitulo() {
      ...
      const textCanton = arregloTexto ? arregloTexto.find(datosCanton => datosCanton.idcan === this.id_codMapa[3]) : null;
      let condicionTexto = this.select2 != "" && textCanton ? textCanton.nombre : textoTitulo.provincia;
    ...
    selectProv() {
      ...
      this.auxpartido2 = this.candidatos.slice(0, 3).map(c => c.nombrePartido); 

      if (this.type == '1' || this.type == '2') {
        this.arregloPartido = this.type == '1' ? this.auxpartido : this.auxpartido2;

        this.aux_prov = this.arreglo_prov.map(p => p.provincia);
        this.provs = this.aux_prov;

        const selectedProvData = this.arreglo_prov.find(p => p.provincia === this.select);
        if (selectedProvData) {
